      ...
      <div className="container-custom">
        ...
        <div className="mb-6 flex flex-wrap gap-2 justify-center">
          {['all', 'won', 'lost', 'draw', 'tie'].map((result) => (
            ...
            >
              {result === 'all' ? 'All Matches' : result.charAt(0).toUpperCase() + result.slice(1)}
            </button>
          ))}
        </div>
        ...
        {filteredMatches.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-2xl font-semibold text-gray-600 mb-2">No Match Results</h2>
            <p className="text-gray-500">
              {isAdminUser ? 'Click "Add Result" to add a match result!' : 'Check back soon for match results.'}
            </p>
          </div>
        ) : (
...
function MatchCard({ match, isAdmin, onUpdate }: { match: PreviousMatch; isAdmin: boolean; onUpdate: () => void }) {
    ...
    <div className="group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden">
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 animate-scale-in">
            ...
            <h3 className="text-xl font-bold text-center mb-2">Delete Match Result?</h3>
      ...
      <div className={`bg-gradient-to-br ${resultColors[match.result]} p-6 text-white ${isAdmin ? 'pr-16' : ''}`}>
        <div className="flex items-center justify-between mb-4 gap-3">
          ...
          <span className={`px-3 py-1 rounded-full text-sm font-bold ${resultBadgeColors[match.result]}`}>
...
function EditMatchModal({ match, onClose, onSuccess }: { match: PreviousMatch; onClose: () => void; onSuccess: () => void }) {
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, result: e.target.value as 'won' | 'lost' | 'draw' | 'tie' })}
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            >
