...
const dummyData = {
  settings: {
    ...
    description: 'A passionate cricket team dedicated to excellence both on and off the field.',
  ...
  if (!supabase) {
    return [
      ...
      },
      {
        ...
        match_date: new Date(Date.now() - 20 * 24 * 60 * 60 * 1000).toISOString(),
      },
    ]
  }
    ...
    return [
      {
        ...
        excerpt: 'Thunder Cricket Club secures a thrilling victory in the quarter finals with an outstanding team performance.',
      ...
      },
      {
        ...
        excerpt: 'We are excited to welcome three talented players to our squad for the upcoming season.',
      ...
      },
      {
        ...
        excerpt: 'Pre-season training camp dates have been finalized. All players are expected to attend.',
...
export async function getStats() {
  ...
  const [playersCount, upcomingCount, wonCount, totalCount] = await Promise.all([
    ...
    supabase.from('matches').select('*', { count: 'exact', head: true }).gte('match_date', new Date().toISOString()),
    supabase.from('previous_matches').select('*', { count: 'exact', head: true }).eq('result', 'won'),
    supabase.from('previous_matches').select('*', { count: 'exact', head: true }),
  ])
